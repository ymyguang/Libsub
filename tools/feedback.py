...
def feedback(text, case='M'):
    ...
    if status is False:
        ...
        if flag == -1:
            ...
            # server酱推送
            print(printLog.get_time("feedback"), "推送状态：失败 -> 当前已进入Server酱推送")
            params = {
                "title": text,
            }
            ...
            if str(s.json()['message']).find("超过当天的发") != -1:
                # 最后推送
                print(printLog.get_time("feedback"), "执行最后coolPush推送")
                t = requests.post("https://push.xuthus.cc/ww/ce4e2dfe9a211ca36f718441f089a88c",
                                  data=text.encode("utf-8"))
                status = t.json()['message']
                print(printLog.get_time("feedback"), "coolPush推送状态:", status)

        else:
            print(printLog.get_time("feedback"), "推送状态：成功")
